...
Acesso transversal de federações/confederações deve ocorrer por vínculos e permissões explícitas, sem romper isolamento lógico.
...
Para dados que afetem enquadramento, cálculo ou cobrança, devem ser priorizadas fontes oficiais e juridicamente acessíveis.

Fontes-alvo incluem, conforme disponibilidade e autorização:

- CNPJ;
- RAIS;
- GFIP;
- eSocial;
- outras bases oficiais pertinentes.

A implementação deve validar legalidade, disponibilidade técnica, finalidade, credenciais e nível de acesso de cada fonte antes de assumir integração.
