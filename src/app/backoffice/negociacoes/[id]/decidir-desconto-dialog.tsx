...
import { decidirDescontoNegociacaoAction, type DecidirDescontoState } from "../actions";
...
export function DecidirDescontoDialog({
  ...
  const [state, formAction, isPending] = useActionState(decidirDescontoNegociacaoAction, initialState);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <DialogHeader>
            <DialogTitle>{aprovado ? "Aprovar desconto" : "Rejeitar desconto"}</DialogTitle>
            <DialogDescription>
              {aprovado
