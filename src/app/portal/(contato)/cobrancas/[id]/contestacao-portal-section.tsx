...
import {
  ...
  adicionarDocumentoEvidenciaPortalAction,
...
function AbrirContestacaoDialog({ cobrancaId }: { cobrancaId: string }) {
  ...
  useEffect(() => {
    if (state.success) {
      toast.success("Contestação registrada — a cobrança fica pausada até a análise da GSBC.");
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <DialogHeader>
            <DialogTitle>Contestar esta cobrança</DialogTitle>
            <DialogDescription>
              Descreva o motivo — a GSBC vai analisar e retornar. A cobrança fica
              pausada enquanto a contestação estiver em aberto.
            </DialogDescription>
          </DialogHeader>

          <div className="flex flex-col gap-2">
            <Label htmlFor="tipoSelect">Motivo *</Label>
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="valorAlegado">Valor que considera correto (se aplicável)</Label>
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPending}>
              {isPending ? "Enviando..." : "Enviar contestação"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
      ...
      <DialogContent>
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <DialogHeader>
            ...
            <DialogDescription>Fica anexado à sua contestação.</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-2">
...
export function ContestacaoPortalSection({
  cobrancaId,
  contestacao,
  eventos,
  evidencias,
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <CardTitle className="text-sm font-medium">Contestação</CardTitle>
        {podeAbrirNova ? <AbrirContestacaoDialog cobrancaId={cobrancaId} /> : null}
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        {!contestacao ? (
          <EmptyState
            ...
            description="Se você discorda do enquadramento, valor, período ou qualquer outro aspecto desta cobrança, pode contestar aqui."
          />
        ) : (
          <>
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                Motivo: {TIPO_LABEL[contestacao.tipo] ?? contestacao.tipo}
              </span>
              {contestacao.valor_alegado ? (
                <span className="text-muted-foreground">
                  Valor considerado correto: {formatCurrency(contestacao.valor_alegado)}
                </span>
              ) : null}
            </div>
                  ...
                  {evidencias.map((ev) => (
                    ...
                    >
                      <div className="flex flex-col">
                        <span>{ev.tipo === "documento" ? ev.documentoNome : ev.comentario}</span>
            ...
            <div>
              <h3 className="mb-2 text-xs font-medium text-muted-foreground">Histórico</h3>
