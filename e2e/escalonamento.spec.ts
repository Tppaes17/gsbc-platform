...
async function expectOk<T>(
  promise: PromiseLike<{ error: { message: string } | null; data?: T }>,
) {
...
async function setupEscalonamentoFixture() {
  ...
  await expectOk(
    admin.from("empresas").insert({
      id: empresaId,
      tenant_id: tenantId,
      razao_social: `Empresa Escalonamento ${suffix}`,
      cnpj: `93${cnpjDigits}`,
      endereco: {
        logradouro: "Rua do Teste, 100",
        cidade: "São Paulo",
        uf: "SP",
      },
      status: "active",
  ...
  await expectOk(
    admin.from("instrumentos").insert({
      id: instrumentoId,
      tenant_id: tenantId,
...
async function cleanupEscalonamentoFixture(
  fixture: Awaited<ReturnType<typeof setupEscalonamentoFixture>>,
) {
  const admin = adminClient();
  ...
  const escalonamentoIds = (escalonamentos ?? []).map(
    (escalonamento) => escalonamento.id,
  );
  ...
  if (escalonamentoIds.length > 0) {
    await admin
      ...
      .delete()
      .in("escalonamento_id", escalonamentoIds);
    await admin
...
test("staff GSBC vê a seção de escalonamento na ficha da cobrança", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Escalonamento e notificação extrajudicial").first(),
  ).toBeVisible();
});
...
test("staff e sindicato veem a Central de Escalonamentos", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Escalonamentos" }),
  ).toBeVisible();
});
...
test("nav lateral tem o item Escalonamentos", async ({ page }) => {
  await loginAs(page, STAFF_EMAIL);
  await expect(
    page.getByRole("link", { name: "Escalonamentos" }),
  ).toBeVisible();
});
...
test("staff executa rejeição, nova aprovação, documento e envio físico com evidência auditável", async ({
  ...
  try {
    ...
    await expect(page.getByText("Nenhum envio nesta etapa")).toBeVisible();
    await page
      .getByLabel("Motivo (critérios de escalonamento) *")
      .fill("Régua automática esgotada sem pagamento ou negociação válida.");
    await page
      .getByRole("button", { name: "Abrir revisão", exact: true })
      .click();
    ...
    await expect(
      page.getByText("Não envia comunicação nesta etapa"),
    ).toBeVisible();
    await page.getByRole("button", { name: "Rejeitar" }).click();
    await page
      .getByLabel("Justificativa *")
      .fill("Faltava evidência suficiente para envio formal.");
    ...
    await page
      .getByLabel("Justificativa *")
      .fill("Critérios documentais atendidos para notificação extrajudicial.");
    await page.getByRole("button", { name: "Confirmar decisão" }).click();
    await expect(page.getByText("Aprovada", { exact: true })).toBeVisible();

    await page.getByRole("button", { name: "Gerar documento (PDF)" }).click();
    await expect(page.getByText("Não envia a notificação")).toBeVisible();
    await page
    ...
    await expect(
      page.getByText("Sem evidência suficiente, a entrega não fica válida"),
    ).toBeVisible();
    await page
      .getByLabel("Destinatário/endereço *")
      .fill("Empresa Escalonamento, Rua do Teste, 100");
    await page.getByLabel("Referência externa").fill("AR E2E-STG09-001");
    await page
      .getByLabel("Observação")
      .fill(
        "Comprovante físico recebido no balcão e protocolo externo informado.",
      );
    await page
      .getByRole("button", { name: "Registrar evidência", exact: true })
      .click();
    ...
    const { data: envios, error: enviosError } = await admin
      .from("escalonamento_envios")
      .select(
        "canal, delivery_status, evidencia_referencia, comprovante_documento_id, delivery_policy_version, policy_relevant_timestamp, delivery_valid",
      )
...
test("envio com falha não avança cobrança para legal_escalation", async () => {
  ...
  try {
    await expectOk(
      admin.from("escalonamentos").insert({
        id: escalonamentoId,
    ...
    const { data: envioId, error: envioError } = await staff.rpc(
      "registrar_envio",
      {
        ...
        p_observacao: "Falha proposital para válidação de transição.",
      },
    );
...
test("policy de delivery bloqueia envio físico sem evidência obrigatória", async () => {
  ...
  try {
    ...
    expect(envioError?.message).toContain(
      "exige comprovante anexado ou referência externa auditável",
    );
...
test("policy de delivery preserva versão, timestamp e vínculo de arquivo histórico", async () => {
  ...
  try {
    ...
    const { data: envio, error: envioSelectError } = await admin
      .from("escalonamento_envios")
      .select(
        "delivery_policy_id, delivery_policy_version, policy_relevant_timestamp, delivery_valid, comprovante_documento_id",
      )
      .eq("id", envioId!)
      .single();
    ...
    await expectOk(
      admin.from("delivery_evidence_policies").insert({
        id: randomUUID(),
        tenant_id: fixture.tenantId,
        version: 2,
    ...
    const { data: unchanged, error: unchangedError } = await admin
      .from("escalonamento_envios")
      .select(
        "delivery_policy_version, policy_relevant_timestamp, comprovante_documento_id",
      )
      .eq("id", envioId!)
      .single();
    ...
    expect(collectionPolicies?.map((policy) => policy.channel).sort()).toEqual([
      "email",
      "whatsapp",
    ]);
  } finally {
